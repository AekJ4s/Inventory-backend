...
public class ProductController : ControllerBase
{
    ...
    public ActionResult CreateProduct([FromBody] ProductCreate productCreate)
    {
    ...
    public ActionResult PUT([FromBody] ProductUpdate product)
    {
        ...
        bool employeeExists = _db.Products.Any(e => e.Id == product.Id && e.IsDeleted != true);
        int IntofData = _db.Products.Where(e => e.Id == product.Id).AsNoTracking().ToList().Count();

        
        if (IntofData > 0)
        {
            Product DataProduct = _db.Products.Where(e =>e.Id == product.Id).AsNoTracking().ToList().First();
            
             newProduct.Name = (newProduct.Name == null || newProduct.Name == "string") ? DataProduct.Name : newProduct.Name;
             
             newProduct.Description = (newProduct.Description == null || newProduct.Description == "string") ? DataProduct.Description : newProduct.Description;

             newProduct.CategoryId = (newProduct.CategoryId == null || newProduct.CategoryId == 0) ? DataProduct.CategoryId : newProduct.CategoryId;

             newProduct.StockQuantity = (newProduct.StockQuantity == null || newProduct.StockQuantity == 0) ? DataProduct.StockQuantity : newProduct.StockQuantity;

             newProduct.Price = (newProduct.Price == null || newProduct.StockQuantity == 0) ? DataProduct.StockQuantity : newProduct.StockQuantity;



            newProduct.IsDeleted = DataProduct.IsDeleted;
            newProduct.CreateDate = DataProduct.CreateDate;
            newProduct.UpdateDate = DateTime.Now;
            newProduct = Product.Update(_db, newProduct);
        }
        else
        {
            return BadRequest();
